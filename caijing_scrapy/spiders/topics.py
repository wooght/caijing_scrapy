...
class TopicsSpider(CrawlSpider):
    ...
    rules = (
        # 雪球头条文章
        Rule(LinkExtractor(allow=('\/\d+\/\d+',), deny=('.*\.jrj.*', '.*\.htm', ',*\.shtml$')), callback='parse_xueqiu',
             follow=True, process_links='link_screen'),
        ...
        Rule(LinkExtractor(allow=('com\.cn\/list\/zjh\-\d+\.shtml$',)), callback='parse', follow=True,
             process_links='link_screen', process_request='wnews_request'),
        # 金融界谈股论金导航页 http://opinion.jrj.com.cn/list/tglj-3.shtml
        Rule(LinkExtractor(allow=('com\.cn\/list\/tglj\-\d+\.shtml$',)), callback='parse', follow=True,
             process_links='link_screen', process_request='wnews_request'),
        # 金融界详情页 http://opinion.jrj.com.cn/2017/11/16080923394008.shtml
        Rule(LinkExtractor(allow=('.*\.com\.cn\/\d+\/\d+\/\d+\.shtml$',)), callback='parse_jrj', follow=False,
